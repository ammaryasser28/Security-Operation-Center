...
- [Step 9 — Prepare Extra Datasets](#step-9--prepare-extra-datasets)
...
## Step 9 — Prepare Extra Datasets

**الهدف من الخطوة دي:** تجهز البيانات الإضافية اللي هتحتاجها في Labs جاية — بعضها مطلوب قبل Lab 1 مباشرة، وبعضها ممكن تستناه.

| Dataset | الغرض منه | إمتى تجيبه |
|---|---|---|
| **BOTS v3** | الـ Incident الأساسي المُقيَّم | خلاص خلصناه في Step 7 — مطلوب لكل الـ Labs |
...
| **BOTS v1 (اختياري)** | سيناريو Web-attack إضافي للتدريب الزيادة | اختياري تمامًا |

> [!NOTE]
> مش لازم تحمّل كل حاجة دلوقتي. الـ Datasets دي صغيرة، وتقدر تحملها بالليل قبل الـ Lab اللي هتستخدمها فيه مباشرة.

### 9.1 Splunk Tutorial Data (قبل Lab 1)

1. افتح صفحة الـ Splunk Search Tutorial ونزّل `tutorialdata.zip`.
2. **متفكّوش** — Splunk بياخد الـ `.zip` مباشرة كما هو.
3. احفظه في مكان تفتكره (Downloads مثلًا)؛ هترفعه في Lab 1 من `Settings → Add Data → Upload`.

### 9.2 loghub (قبل Lab 1 و Lab 5)

1. افتح صفحة تحميل loghub.
2. نزّل 3 عينات صغيرة بس: **Linux log**، **OpenSSH log**، **Apache log** (نصوص عادية صغيرة).
3. احفظهم جنب بعض؛ Linux/SSH هتستخدمها في Lab 1، وApache في Lab 5.

### 9.3 attack_data (قبل Lab 6)

1. افتح `github.com/splunk/attack_data` وشوف شكل الـ Repo بس، مش لازم تحمله كامل.
2. احفظ اسم تقنية زي `T1059.001` (PowerShell خبيث) هتستخدمها في Lab 6.

### 9.4 BOTS v1 (اختياري)

لو عايز تدريب إضافي وعندك وقت ومساحة، نصّبه بنفس طريقة BOTS v3 في Step 7، من `github.com/splunk/botsv1`. غير كده، تجاهله تمامًا.

> [!TIP]
> **متحملش الـ Universal Forwarder دلوقتي.** Lab 1 هيوريك بالظبط إمتى وإزاي تحمله وتنصبه.

---

## Troubleshooting Reference

| Symptom | السبب المحتمل | الحل |
|---|---|---|
| VM مش شغالة / "VT-x disabled" | Virtualization مش مفعّلة | فعّلها من BIOS/UEFI وأعد تشغيل الـ Host |
| VM بطيئة جدًا | الـ vCPU قليل أو الـ Virtualization مش مفعّلة كامل | تأكد من التفعيل، وزود الـ vCPU لـ 4 من إعدادات الـ VM |
| شاشة سودا / دقة غلط | `open-vm-tools-desktop` مش منصب | نصّبه (Step 5) واعمل Reboot |
| مفيش Clipboard/Drag-drop | نفس السبب فوق | نصّب `open-vm-tools-desktop` وأعد التشغيل |
| BOTS بيرجع 0 Events | Time Picker على "Last 24 hours" | غيّره لـ **All time** أو ضيف `earliest=0` |
| Fields أو Sourcetype ناقصة | Add-on غير منصب أو Restart متعملش | نصّب الـ Add-on المناسب وأعد تشغيل Splunk |
| مش قادر توصل `localhost:8000` | Splunk واقف | نفّذ `sudo /opt/splunk/bin/splunk start --run-as-root` |
| "Permission denied" على log/pid files | Ownership Mismatch | اعمل الـ Full Reset الموضح في Step 6 |
| "Not Entitled" على Broadcom | مش واقف في القسم الصح | ادخل `VMware Cloud Foundation` → Free Downloads |
